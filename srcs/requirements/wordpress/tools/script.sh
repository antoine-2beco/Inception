...
if [ ! -f "/var/www/html/wp-config.php" ]; then
    ...
    wp core install --allow-root --url="${NGINX_HOST}" --title="${WP_TITLE}" --admin_user="${WP_ADMIN_USER}" --admin_password="${WP_ADMIN_PWD}" --admin_email="${WP_ADMIN_EMAIL}" --path="/var/www/html" --skip-email
